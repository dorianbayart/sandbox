...
const state = {
    racers: {
        ...
        ai1: {  // AI-1
            ...
            height: 60,
            speed: 0,
            minSpeed: 27.777 * 2, // 10 km/h
            maxSpeed: 833.333, // 300 km/h
            speedMargin: 1.01,//1.01,
            acceleration: 150,
            deceleration: 300,
            turnSpeed: 300,
            baseSpeed: 400,
            ...
            network: null, // Will be assigned by trainign system
            layers: [10, 10, 8, 2], // balanced
        },
        ai2: {  // AI-2
            name: 'AI-2',
            color: '#6666cc',
...
function updateAI(ai, deltaTime) {
    if(!ai.network) return;

    const inputs = RaceAIUtils.getNeuralNetworkInputs(ai, state.road, Object.values(state.racers));
    const controls = ai.network.getCarControls(inputs);
    
    ai.targetAngle = controls.targetAngle;
    const targetSpeed = ai.minSpeed + (controls.targetSpeed + 1) * 0.5 * (ai.maxSpeed - ai.minSpeed);


    if(ai.x + ai.width/2 < canvas.width * 0.1) ai.targetAngle = Math.PI / 6;
    if(canvas.width - ai.x - ai.width/2 < canvas.width * 0.1) ai.targetAngle = -Math.PI / 6;
    ...
    ai.x += Math.sin(ai.angle) * ai.turnSpeed * deltaTime * turnMultiplier;
    
    // Keep on screen
    //if(ai.x - ai.width > canvas.width * 0.95 || ai.x + ai.width < canvas.width * 0.05) ai.angle = 0;
    ai.x = Math.max(ai.width/2, Math.min(canvas.width - ai.width/2, ai.x));
    
    
    // Always accelerate when on road
    if (/*Math.abs(distanceFromTarget) < state.road.width * 0.4 &&*/ ai.isOnRoad && ai.speed <= targetSpeed && (canvas.height - ai.y)/canvas.height > ai.yMargin) {
        // On road - accelerate
        ai.speed = Math.min(
        ...
        // Off road - slow down
        const deceleration = ai.isOnRoad ? ai.deceleration : ai.deceleration / 4;
        ai.speed = Math.max(
            ai.minSpeed,
            ai.speed - deceleration * deltaTime
        );
    }

    // if(ai.name === 'AI-1') console.log(
    //     'target speed:' + targetSpeed,
    //     'speed:' + ai.speed,
    //     'target angle:' + ai.targetAngle,
    //     'angle:' + ai.angle,
    // )

    //ai.speed = Math.min(ai.maxSpeed, (9 * ai.maxSpeed + 5 * state.racers.player.speed) / 10);
    ...
    // Check if AI is on road
    ai.isOnRoad = Math.abs(RaceAIUtils.getDistanceFromCenter(ai, state.road)) < state.road.width/2;

     // Update distance if on road
    if(ai.isOnRoad) ai.distanceTraveled += ai.speed * deltaTime;
}

// Initialize road
function initRoad() {
    for (let y = -10*state.road.segmentLength; y < 3 * canvas.height; y += state.road.segmentLength) {
...
function generateRoad() {
    ...
    if (state.road.points[state.road.points.length-1].y - state.camera.y < 25 * canvas.height) {
    ...
    while (state.road.points[0].y - state.camera.y < -state.road.segmentLength * 100) {
...
function drawRacerHUD(racer, position, index) {
    ...
    // Speed indicator
    const speedPercent = Math.min(1, (racer.speed - racer.minSpeed) / (racer.maxSpeed - racer.minSpeed));
...
function update(deltaTime) {
    ...
    if (keys['ArrowLeft']) {
        ...
        state.racers.player.speed -= turnMultiplier * state.racers.player.brakeDeceleration * deltaTime
    ...
    if(state.game.isOnRoad) state.game.distance += state.racers.player.speed * deltaTime / 10; // 1 pixel = 1 cm
...
function gameLoop(currentTime) {
    ...
    Object.entries(state.racers).filter(([id, racer]) => racer.name !== 'YOU').forEach(([id, racer]) => {
        racer.trainingSystem.update({ [id]: racer }, state.road);
...
Object.values(state.racers).filter(racer => racer.name !== 'YOU').forEach(racer => {
    racer.trainingSystem = new AITrainingSystem(racer.layer); // AI cars
});
//const trainingSystem = new AITrainingSystem(Object.values(state.racers).filter(racer => racer.name !== 'YOU')); // AI cars
requestAnimationFrame(gameLoop);
